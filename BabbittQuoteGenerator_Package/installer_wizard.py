...
class InstallerWizard:
    ...
    def install_application(self):
        ...
        if not exe_path.exists():
            messagebox.showerror("Error", f"Could not find {self.app_exe}")
            return
        ...
        def install_thread():
            try:
                ...
                # Copy executable
                shutil.copy2(exe_path, self.install_path / self.app_exe)
                
                # Copy additional files
                additional_files = ["USER_GUIDE.md", "QUICK_DEPLOYMENT_GUIDE.md"]
                for file in additional_files:
                    if Path(file).exists():
                        shutil.copy2(file, self.install_path / file)
